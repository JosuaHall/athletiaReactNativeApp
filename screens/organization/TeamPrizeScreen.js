...
import {
  createLeaderboard,
  deleteTeamLeaderboard,
} from "../../actions/organizationActions";
import DeleteButton from "../../components/DeleteButton";
import { getTeamLeaderboard } from "../../actions/teamActions";
import LoadingSpinnerStackScreen from "../LoadingSpinnerStackScreen";

const TeamPrizeScreen = (navigation) => {
  ...
  const selectedTeam = useSelector((state) => state.team.selected_team);
  const hasLeaderboard = useSelector((state) => state.team.teamLeaderboard);
  const leaderboardLoading = useSelector(
    (state) => state.team.teamLeaderboardLoading
  );
  ...
  useEffect(() => {
    dispatch(getTeamLeaderboard(orgid, teamid));
  }, [selectedTeam]);
  ...
  const setFanPrizes = () => {
    if (startDate && endDate && prizeList.length !== 0) {
      setMsg(null);
      const data = { orgid, teamid, startDate, endDate, prizeList };
  ...
  const handleDeleteLeaderboard = (id) => {
    handleCloseModal();
    dispatch(deleteTeamLeaderboard(id));
    setPrizeList([]);
  };
              ...
              >
                {hasLeaderboard.prizes.map((item, index) => (
                  <View key={index} style={styles.itemContainer}>
                    ...
                    >
                      <Text
                        style={{ color: colors.text, ...styles.itemNumber }}
                      >
                        {item.place}.
                      </Text>
                    </View>
...
export default TeamPrizeScreen;
